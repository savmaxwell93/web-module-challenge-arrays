...
function is31Flavors(array){
 ...
 //conditional here if array.length is 31 return true else return false **exact match
}
...
*/
//2 params - array, flavor-string
function addFlavor(array, flavor){
  array.unshift(flavor);
//add flavor to front of array - unshift
return array;
//return the array
}
...
function removeLastFlavor(array){
 array.pop();
 //remove item at the end of the array - pop
 return array;
 //return the array
}
...
*/
//2 param - array, string to remove
function removeFlavorByName(array, string){
  for(let i = 0; i < array.length; i++){ //loop through the array, check to see if the string is at that index, if it is remove it
    if(array[i] === string){ //looking for exact match
      array.splice(i, 1);
    }
  }
...
function filterByWord(array, string){
  const filteredFlavors = []; //new array to push items to
  for(let i = 0; i < array.length; i++){ //loop thorugh array and check to see if items includes the string
    if(array[i].includes(string)){ //if it does push it to new array
        filteredFlavors.push(array[i]);
    }
  }   
  return filteredFlavors //return new array
}
...
function getAverageWordLength(array){
  ...
  if(array.length === 0){
    ...
    sum = sum + array[i].split([' ']).length; //count how many words per item
  }
